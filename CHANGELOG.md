...
- Typography now follows the reader-typography research (ADR-0033): text is 20 px on a 30 px line, the column holds 66 real characters instead of the 85 that "68 ch" produced, code keeps its authored width into the margin with wrapped lines marked and hung past their own indentation, programming ligatures are off, syntax colour marks only strings, literals, comments and names while comments are no longer dimmed, `ts`/`js`/`sh`/`py` fences are finally highlighted and highlighting no longer freezes the page, hyphenation is used only when a paragraph needs it, and numbered-list items no longer start their first line to the right of the rest (MARXY-199)
