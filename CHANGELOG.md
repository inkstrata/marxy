...
- A file that is not valid UTF-8 no longer gets byte offsets past its own bytes, a character reference such as `&#10;` no longer drops the text after it, footnotes match their definitions whatever the case, copied sections keep reference links, and editing a mixed-ending or Latin-1 file in Source changes only what was edited. Leaving Source after an edit re-renders the page from the new bytes, opening another document (including from the palette) replaces the first one cleanly, a live reload keeps your place when text is inserted above it, background typesetting no longer re-reads the whole page after every few paragraphs (8× fewer layout reads on a 500 KB document), documents cross from the shell as raw bytes instead of JSON number arrays, and recently read files rank higher in the palette (MARXY-198)
...
- The orchestrator adopts every open pull request the board does not have in review, so out-of-plan work and a story whose PR opened without `state.mjs review` reach the merge bar instead of a hand merge; a branch is judged by its own board row, so it can bring its row or widen its own Paths in the same PR while an edit to another story's row is held, and CI's `check-story --strict` applies the same rule and fails a key with no row; a `no-dispatch` row is never sent to an implementor and is recorded Done once its PR merges, even one merged while the cycle was stopped. Out-of-plan work is one command and one PR: `out-of-plan.mjs start` creates the Jira Task, a worktree and the change's own board row; `jira.mjs sync --new` resolves a planner's `MARXY-NEW-` placeholders in its own branch (card files renamed, untracked files included, longest key first) so none reaches main, and `check-cards` fails one that does; `pnpm done --open` pushes the branch before `gh pr create`; the protocol is written into `docs/sdlc.md`, `AGENTS.md` and the prompts (MARXY-190)
...
- The orchestrator keeps `mergeQueue` false on this User-owned repository because GitHub's native merge queue requires an organization owner; one BEHIND pull request is still refreshed per cycle until an org transfer (MARXY-182)
