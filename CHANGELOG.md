...
- Task cards and CSV rows are checked against each other and against `orchestration/deps.json` before every pull request, so implementors and the story-boundary hook read the same spec (MARXY-126)
