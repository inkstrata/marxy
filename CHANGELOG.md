...
- Source mode in CodeMirror 6 under `apps/desktop/src/source/`: per-file-type default, position-preserving mode switch helpers, and byte-faithful leave-Source commit (MARXY-37)
...
- Merged and closed story worktrees are now actually pruned: the prune asked `gh pr list --head` for open PRs only, so every finished branch looked PR-less and was kept, leaving ~45 stale worktrees and a 60-line status report; a story in progress keeps its worktree even when an earlier attempt's PR is closed (MARXY-171)
