...
- The orchestrator human queue records Ian's 2026-09-21 rulings (no Linux desktop yet, ADR-0026 accepted, merge queue yes with the GitHub switch still off), and a leftover `MARXY-NEW-` board cache row is renamed onto its real Jira key so it stops counting as an orphan (MARXY-179)
