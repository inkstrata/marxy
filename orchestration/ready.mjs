// Stories that can start now: deps done, paths disjoint from in-progress work, lane limit respected.
import { state, stories, deps, pathsOf, overlap, models } from './lib.mjs';
...
const free = Math.max(0, models().lanes - busy.length);
...
console.log(JSON.stringify({ lanesFree: free, inProgress: busy.map(b => b.Key), ready: picked.map(p => ({ key: p.Key, summary: p.Summary, paths: p.Paths })), waitingOnDeps: candidates.length - picked.length }, null, 2));
