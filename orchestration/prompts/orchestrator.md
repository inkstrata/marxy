...
Run `node orchestration/doctor.mjs` when you start, when the machine has slept, and whenever the
board looks wrong. It names every problem with the command that fixes it; `--fix` applies the
