...
|---|---|---|---|---|
| Text face | Literata (serif); Atkinson Hyperlegible Next offered as sans | Any bundled or installed face | {{D}} | Designed for screen reading, full features, no RFN ([Fonts](@fonts)) |
| Size | 18px phone, 19px tablet, 20px desktop (in `rem`) | 75–250%, and further for low vision | {{A}} for the floor | x-height at or above about 0.2° ([Evidence](@evidence)) |
| Size across faces | Match x-heights, not point sizes | Automatic | {{D}} | x-heights differ by up to a third ([Fonts](@fonts)) |
| Measure | 66 average characters | 45–80, bounded by the screen | {{C}} | Preference and one comprehension result; no speed optimum ([Spacing](@spacing-layout)) |
| Measure unit | Characters × measured average character width | Automatic | Measured | `ch` gives 75–104 characters for "66ch" ([Fonts](@fonts)) |
| Line height | 1.5, unitless | 1.3–2.0 | {{C}}/{{D}} | Floor near 1.2–1.5; WCAG AAA; matches reading systems ([Spacing](@spacing-layout)) |
| Paragraphs | The book's own style; else 1em indent (books) or 0.75em space (articles) | Indent 0–3rem, spacing 0–2rem, never both | {{D}} | Book convention ([Content types](@content-types)) |
| Letter spacing | Font default (0) | 0–0.5rem; rises with word spacing | {{A}} that default is best for most | Spacing helps only some readers, and only with word spacing ([Evidence](@evidence)) |
| Word spacing | Font default (0) | 0–1rem | as above | WCAG 1.4.12 requires tolerance of 0.16em |
| Alignment | Ragged right, hyphenated | Justified (total-fit breaking) where the measure is at least 45 characters | {{X}} for justification | Greedy justification makes loose lines; no algorithm fixes narrow columns ([Line breaking](@line-breaking)) |
...
| Theme | Follow the system; dark and light offered | Dark, light, plus high contrast | {{A}} for the legibility difference | Dark on light is slightly more legible; comfort and low vision favour choice ([Color & access](@color-access)) |
...
| Page model | Paged for books, scrolling for articles and docs | Either | {{C}} | Small lean toward paging for deep reading ([Spacing](@spacing-layout)) |
| Notes | Pop-up (paged) or margin notes (wide screens) | Also endnotes | {{D}} | Keep notes near their reference ([Content types](@content-types)) |
| Code | Monospace, no ligatures, highlighted at import, scrolls in its own container | Wrap on or off, tab width | {{C}} for highlighting | ([Code](@code)) |
...
From [Multilingual Text Layout](@multilingual):
