...
slug: sources
number: 11
title: Typography Source Ledger
short: Sources
h1: Typography Source Ledger
lede: Every important source behind the handbook, graded for quality, with a note on what each one shows and what it does not. It includes the sources that turned out weaker than their reputations, and the claims the research could not verify, which were left out of the chapters on purpose.
description: Every source the handbook relies on, graded for quality, with what each one does and does not show.
when: you want to check a citation or judge a source
icon: list
---
