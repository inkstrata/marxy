...
> **Trap.** Language detection is not a substitute for tagging. Short strings, names and mixed passages are routinely misdetected, and a misdetected Japanese passage gets Chinese glyphs and Chinese line breaking. Trust explicit tags first, detect only untagged text, and let users correct the language of a book.
