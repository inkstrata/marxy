<!-- Generated from source/01-pipeline.md by source/to-gfm.mjs; edit the source, then run the script. -->

# The Typesetting Pipeline

*Between a string of characters and a page someone can read lie eight distinct stages, each with its own standards, failure modes and libraries. Knowing where a decision is made (the font, the shaper, the line breaker, the rasterizer) is most of the work of getting it right, and it decides which stack a reader should be built on.*
...
This is the subject of [chapter 5](05-line-breaking.md), which explains the algorithm, measures it against Chromium's line breaker, and includes a live comparison.
