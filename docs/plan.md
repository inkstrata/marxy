...
phase story is dispatched first. Phase 0's remaining process work moved there on 2026-09-19,
including the release pipeline (MARXY-16): v0.0.1 ships when it is ready, and Phase 1 does not wait
for it. The rule the lane exists to keep: the loop may improve itself, but never at the cost of the
page.
...
only through a pull request, the planner's included. `orchestration/state.json` and
`orchestration/needs-human.md` are local to the orchestrator's checkout and gitignored: Jira is the
record and state.json its cache, so the orchestrator's `main` stays clean and fast-forwards every
cycle (MARXY-107).
