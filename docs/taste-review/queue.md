...
| 2026-09-25 | MARXY-43 | Toggling a task item and aligning a table's pipes, then undo | `packages/core/src/operations/operations.test.ts` and the desktop command wiring in PR #199 — no screenshot pair; the operations splice source bytes and the page re-renders from them | After a task is toggled and a ragged table is aligned, does the page still read as the same book, and does undo put the marks back? | |
