...
| 2026-09-20 | MARXY-87 | First summoned palette: `Mod+P` opens a top-centre dialog with search and results; chrome at rest stays zero (no tab bar) | `apps/desktop/test/palette.test.mjs` Playwright captures pending — empty query (pins then MRU), typing with Tab between documents and headings; dark, 960 px | Does the summoned palette read as native chrome rather than a web form dropped on the page? Is the list legible at 1× without feeling cramped? | |
