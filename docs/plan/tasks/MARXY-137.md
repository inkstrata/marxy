...
> **Revised 2026-09-19, after PR #104 was returned at `f697884`** —
> `orchestration/results/MARXY-137.notes.md`, `docs/plan/deltas/2026-09-19-marxy-137-headless.md`.
> The tree on that PR is right as far as it goes and **stays open — amend it, do not cut a new
> branch and do not close it.** Two things changed in the row: `Paths` gain
> `apps/desktop/src/render/headless.ts`, and criterion 4 now names that file while a new criterion 8
> pins the two entry points to the same option set. The returned attempt flipped `app.ts` only,
> which left the gate measuring a render path no reader gets. Wait for MARXY-143 to merge before
> you resume: until it does, `gate:aesthetics` names a different document each run on
> `webkit-linux` and criterion 4 cannot be shown green.
...
`headless.ts` plus `gate:aesthetics` and the PR's baseline list, 5 is `gate:perf` with the numbers
pasted, 6 is the artifact and the queue row, 7 is the `CHANGELOG.md` line, 8 is the parity case.
...
font waits are MARXY-143's, and it lands before you. Open a second PR: #104 is the one.
