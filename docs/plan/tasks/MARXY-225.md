...
`../marxy-wt/MARXY-218` is dirty: its uncommitted row lists `orchestration/ready.mjs`. Do not start
until MARXY-223 has merged: both edit `orchestration/README.md`, and until this story lands
`ready.mjs` will not report that overlap. Cut it from `origin/main` after that. MARXY-220 depends
on this story and must not start first.
...
todo`). The worktree `../marxy-wt/MARXY-195` is dirty and ahead 0. Status is `todo`, attempts is 1.
`ready.mjs` then excludes MARXY-195 with `worktree holds` by MARXY-195, and the cycle only
dispatches keys in `ready`. The same filter drops a claim whose status is `in_review` even when
that key has no row in the CSV the cycle reads. MARXY-223 is `in_review` in `state.json` and its
row exists only on its branch, so #207 holds `orchestration/cycle.mjs` and `orchestration/README.md`
on disk and holds nothing in `ready.mjs`.
...
- `orchestration/ready.test.mjs` — the two cases below. The test `a claim whose key is already
  in_progress does not change ready` stays green and is not rewritten.
...
| `all` is only MARXY-220 (`orchestration/cycle.mjs`, `todo`); `s.stories['MARXY-223']` is `{ status: 'in_review' }` and MARXY-223 is not in `all`; `claims` is one claim for MARXY-223 on `orchestration/cycle.mjs` | MARXY-220 is not in `ready`; `excluded` names `worktree holds` by MARXY-223 |
...
## Do not
- Change `liveClaims` in `worktrees.mjs`. Skipping `blocked` and `escalate` is MARXY-220, and it depends on this story.
