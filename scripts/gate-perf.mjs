...
// held to no ceiling until MARXY-70 derives one. `parse_long_technical_ms` goes through the same CI
// rule as the warm start, from the same budgets file, and is required in both tiers. Reads
// fixtures/perf-budgets.json and results/perf.json (startup from scripts/measure-startup.mjs; parse
...
// measure-startup.mjs replaces results/perf.json wholesale, so the parse test also leaves its number
// in a snapshot and the gate merges it back. Pure so --selftest can drive it without touching disk.
...
export function evaluate({ envClass, budgets, results, runnerClass }) {
  ...
  // regresses, because the parse number reaches here through the test, a file it does not own and a
  // merge, and any broken link would otherwise leave a green run that never mentions the metric.
...
// A pull request may add a budget; it may not quietly loosen one. So the comparison is asymmetric:
// every number the older revision carried must still be there with the same value, and keys that did
// not exist before are allowed. Byte equality would have said the same thing until a metric was added.
...
function selftest() {
  ...
  for (const [story, names] of [['MARXY-55', MARXY_55_CASE_NAMES], ['MARXY-59', MARXY_59_CASE_NAMES]]) {
    for (const name of names) {
      if (!SELFTEST_CASES.some(c => c.name === name)) { bad++; console.error(`selftest FAIL: the ${story} case "${name}" is no longer in the suite`); }
    }
  }

  // The parse number survives measure-startup.mjs replacing results/perf.json only through this merge,
  // so the merge is checked here rather than trusted.
  for (const c of [
  ...
  // A ci parse entry must be arithmetic on a number that was actually observed. `derived_from` is the
  // human audit trail for *which* run that was — it is recorded, not machine-verified against CI, and
  // re-deriving the baselines from a defensible statistic is MARXY-70's job for every class at once.
  for (const cls of ['ubuntu-latest', 'macos-latest']) {
    ...
    const ok = Boolean(entry) && median != null && entry.multiplier === derived && entry.baseline_ms === median;
