...
test('no tab bar exists when the palette is dismissed or summoned', () => {
  ...
  assert.equal(view.root.getAttribute('id'), 'marxy-palette');
  ...
  assert.equal(host.querySelector('#marxy-palette')?.getAttribute('role'), 'dialog');
  assert.equal(host.querySelector('.marxy-palette-results')?.tagName, 'OL');
  assert.equal(host.querySelector('[data-marxy-s]')?.getAttribute('data-marxy-s'), '240');
  assert.equal(hasTabBar(host), false);
  assert.equal(host.querySelector('[role="tablist"]'), null);
  assert.equal(host.querySelector('[role="tab"]'), null);
  ...
  view.destroy();
  assert.equal(host.querySelector('#marxy-palette'), null);
  assert.equal(hasTabBar(host), false);
});
