...
export function mountPalette(
  ...
  root.setAttribute('id', 'marxy-palette');
  ...
  const paint = (next: PaletteViewState) => {
    ...
    for (let i = 0; i < next.hits.length; i++) {
      ...
      item.setAttribute('class', 'marxy-palette-row');
      if (hit.heading !== undefined) {
        const heading = hit.entry.headings[hit.heading];
        if (heading !== undefined) {
          item.setAttribute('data-marxy-s', String(heading.byteOffset));
        }
      }
      if (i === next.selected) item.setAttribute('aria-selected', 'true');
      const label = doc.createElement('span');
      label.setAttribute('class', 'marxy-palette-title');
      ...
      if (hit.heading !== undefined) {
        ...
        if (heading !== undefined) {
          ...
          mark.setAttribute('class', 'marxy-palette-heading');
    ...
    if (next.notice !== undefined && next.notice.length > 0) {
      const note = doc.createElement('p');
      note.setAttribute('class', 'marxy-palette-notice');
      setText(note, next.notice);
